...
import { Download, File, FileImage, FileText, FileVideo, Trash2, Upload } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="font-bold">{t('fileName')}</TableHead>
            <TableHead className="font-bold">{t('type')}</TableHead>
            <TableHead className="font-bold">{t('uploaded')}</TableHead>
        ...
        <TableBody>
          {media.length === 0 ? (
            ...
            media.map((asset, index) => (
              <TableRow
                key={asset.id}
                className={cn(index % 2 === 0 ? 'bg-surface-page' : 'bg-surface-image-background')}
              >
                <TableCell className="px-2 py-4">
                  <div className="flex items-center gap-2">
                    <FileTypeIcon contentType={asset.contentType} />
                    <span className="font-medium truncate max-w-xs">{asset.fileName}</span>
                  </div>
                </TableCell>
                <TableCell className="px-2 py-4 text-sm text-text-secondary">{asset.contentType}</TableCell>
                <TableCell className="px-2 py-4 text-sm text-text-secondary">
