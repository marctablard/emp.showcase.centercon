  ...
  return (
    <>
      ...
      <div className="hidden md:block fixed top-0 left-0 right-0 pt-4 z-50 max-w-6xl mx-auto">
        ...
        >
          <div
            className={`transition-all duration-200 ease-in-out absolute top-0 left-0 right-0 w-full ${scrolled ? 'opacity-0 transform -translate-y-4 z-1' : 'opacity-100 z-2'}`}
          >
            <HeaderExpanded />
          </div>

          <div
            className={`flex justify-between items-center transition-all duration-200 ease-in-out absolute top-0 left-0 right-0 h-[64px] px-6 ${scrolled ? 'opacity-100 transform translate-y-0 z-2' : 'opacity-0 transform translate-y-4 z-1'}`}
          >
            <HeaderCollapsed />
          </div>
        </header>
