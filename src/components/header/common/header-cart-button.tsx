...
export default function HeaderCartButton({ initialCart, showSum = true }: HeaderCartButtonProps) {
  ...
  return (
    <NavigationMenu viewport={false}>
      <NavigationMenuList>
        <NavigationMenuItem>
          <MiniCartMenuTrigger className="pl-[11px] md:pl-4 pr-1 pb-2 pt-1 md:py-1 gap-4 self-center" ref={menuTrigger}>
            {showSum && (
          ...
          <MiniCartMenuContent>
            {loading ? (
              ...
              <div className="flex flex-col gap-4 justify-center">
                <div className="overflow-y-scroll max-h-[300px] pr-0.5">
                  {cart.items.map((item) => (
                    <div key={item.id} className={`py-4 border-b flex items-end justify-between gap-3`}>
                      ...
                      <div>
                        {cartUpdate && cartUpdate?.itemId === item.id && (
                          <Badge
                            variant="warning"
                            className="h-5 min-w-5 rounded-full px-1 tabular-nums tracking-normal"
                          >
                            <MessageCircleWarning />
                          </Badge>
                        )}
