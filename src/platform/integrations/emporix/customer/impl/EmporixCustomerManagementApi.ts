...
class EmporixCustomerManagementApi implements IEmporixCustomerManagementApi {
  ...
  async getContactAssignmentsByCustomerId(customerId: string): Promise<EmporixContactAssignment[]> {
    const url = `customer-management/${this.config.tenant}/contact-assignments?customer.id=${encodeURIComponent(customerId)}&pageSize=100`;
    const response = await this.apiClient.authenticatedFetch(
      url,
      { method: 'GET' },
      'service',
      undefined,
      createCustomerMgmtMetrics('/customer-management/{tenant}/contact-assignments'),
    );
    if (!response.ok)
  ...
  async getContactAssignmentsByLegalEntityId(legalEntityId: string): Promise<EmporixContactAssignment[]> {
    const url = `customer-management/${this.config.tenant}/contact-assignments?legalEntity.id=${encodeURIComponent(legalEntityId)}&pageSize=100`;
    const response = await this.apiClient.authenticatedFetch(
      url,
      { method: 'GET' },
      'service',
  ...
  async createLegalEntity(data: EmporixLegalEntity): Promise<EmporixLegalEntity> {
    const url = `customer-management/${this.config.tenant}/legal-entities`;
    const response = await this.apiClient.authenticatedFetch(
      url,
      { method: 'POST', body: JSON.stringify(data), headers: { 'Content-Type': 'application/json' } },
      'session',
      undefined,
      createCustomerMgmtMetrics('/customer-management/{tenant}/legal-entities'),
    );
  ...
  async updateLegalEntity(id: string, legalEntity: EmporixLegalEntity): Promise<EmporixLegalEntity> {
    const url = `customer-management/${this.config.tenant}/legal-entities/${id}`;
    const response = await this.apiClient.authenticatedFetch(
      url,
  ...
  async getContactAssignmentById(contactAssignmentId: string): Promise<EmporixContactAssignment> {
    const url = `customer-management/${this.config.tenant}/contact-assignments/${contactAssignmentId}`;
    const response = await this.apiClient.authenticatedFetch(
      url,
      { method: 'GET' },
      'session',
      undefined,
      createCustomerMgmtMetrics('/customer-management/{tenant}/contact-assignments/{id}'),
    );
  ...
  async createLocation(data: EmporixLocation): Promise<EmporixLocation> {
    const url = `customer-management/${this.config.tenant}/locations`;
    const response = await this.apiClient.authenticatedFetch(
      url,
      { method: 'POST', body: JSON.stringify(data), headers: { 'Content-Type': 'application/json' } },
      'session',
      undefined,
      createCustomerMgmtMetrics('/customer-management/{tenant}/locations'),
    );
  ...
  async getLocationById(locationId: string): Promise<Location> {
    const url = `customer-management/${this.config.tenant}/locations/${locationId}`;
    const response = await this.apiClient.authenticatedFetch(
      url,
      { method: 'GET' },
      'service',
      undefined,
      createCustomerMgmtMetrics('/customer-management/{tenant}/locations/{id}'),
    );
